...
void train(short piece, bool* list_of_features[], double* theta, bool reference[]) {
    ...
    int count = 0;
    while ((cost = cost_function(list_of_features, theta, reference)) < prev_cost - .1 && count++ < TRAIN_MAX_TIMES) {
        prev_cost = cost;
...
void predict(short piece, double theta[]) {
    ...
    while (fscanf(test_data, "%d", &index) != EOF) {
        while (fscanf(test_data, "%d:1", &feature)) {
            predict_feature[feature_map[feature]] = 1; // All feature comes from test, so must in feature map
            if ((c = fgetc(test_data)) == '\n')
                break;
        }
        predict_result = 0;
...
// Read theta and predict
void read_and_predict(short piece) {
    bool* list_of_features[TRAIN_PIECE_SIZE];
    for (int i = 0; i < TRAIN_PIECE_SIZE; i++) {
        list_of_features[i] = new bool[feature_actual];
        list_of_features[i][0] = true; // The first feature, always be 1
    ...
    std::ifstream theta_data(_ss.str().c_str());
    double d;
    while (++count < feature_actual) {
      theta_data >> d;
      theta[count] = d;
    }
...
void combine_predict() {
    ...
    short list_of_predicts[TEST_SIZE];
